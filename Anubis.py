...
class text_widget(QWidget):
    def __init__(self, isPyhton):
        super().__init__()
        self.itUI(isPyhton)
    def itUI(self, isPyhton):
        global text
        text = QTextEdit()
        if isPyhton == 0:
...
class Widget(QWidget):
    UI=0
    def __init__(self, isPyhton, UI):
        super().__init__()
        self.UI = UI
        self.initUI(isPyhton)

    def initUI(self, isPyhton):

        # This widget is responsible of making Tab in IDE which makes the Text editor looks nice
        tab = QTabWidget()
        tx = text_widget(isPyhton)
        tab.addTab(tx, "Tab"+"1")
    ...
    def on_clicked(self, index):
        ...
        url = QUrl.fromLocalFile(nn)
        filename = QFileInfo(nn).fileName()
        if filename[-2:] == 'cs' or filename[-2:] == 'CS':
            self.UI.lanC()
        elif filename[-2:] == 'py' or filename[-2:] == 'PY':
            self.UI.lanP()
        if nn:
            f = open(nn,'r')
            with f:
...
#        Port.addAction(Port_Action)
        ...
        Python_action.triggered.connect(self.lanP)
        Language.addAction(Python_action)
        Cs_action = QAction("C#", self)
        Cs_action.triggered.connect(self.lanC)
        Language.addAction(Cs_action)
        ...
        widget = Widget(self.isPyhton, self)
...
#            ide.upload_file(self.portNo)
    ...
    def open(self):
        ...
        url = QUrl.fromLocalFile(file_name[0])
